...
- 宣言importの正規化: 生成された宣言ファイルのimportをNodeNext互換の形に補正し、`.d.ts.map` も追従して補正します。
...
ソースコードでは、bundler に解決させる前提で相対importの拡張子を省略したい場合があります。
一方、公開された宣言ファイルは TypeScript から直接参照されるため、`moduleResolution: "node16"` / `"nodenext"` では最終的な JavaScript モジュール名に一致するspecifierが必要です。

screw-up は有効時、生成された `.d.ts` / `.d.mts` / `.d.cts` 内の相対importを、実際の出力に合わせて以下の拡張子へ補正します:
...
ディレクトリ解決が必要な場合も、実際に生成された宣言ファイルを見て `./nested/index.js` のように補正します。
パッケージ名のような bare specifier は変更せず、対応する `.d.ts.map` も生成列位置がずれないように補正します。

なお、この機能を無効化する場合は、オプションに `fixDeclarationImportExtensions: false` を指定してください。
...
`outputMetadataFile`を`true`にすると、`packageMetadata.ts`が生成されるようになります。デフォルトでは`packageMetadata.ts`にGitコミットIDが含まれるため、コミット毎にファイルが更新されます。現在のscrew-upはメタデータファイルと同じディレクトリに`.gitignore`が存在しない場合、自動的に作成して除外設定を行います。ただし、生成先を変更した場合などは`.gitignore`が維持されていることを確認してください。
