...
class GitTestRepository {
  ...
  readonly createAnnotatedTag = async (
    tagName: string,
    message: string,
    commitHash?: string,
  ): Promise<void> => {
  ...
  readonly getCommit = async (hash: string): Promise<any> => {
    ...
    return {
      ...
      message: lines[3],
      parents: lines[4]
        ? lines[4].split(" ").filter((p: string) => p.length > 0)
        : [],
    };
  };
}
...
const cleanupTestRepository = async (
  ...
  describe("git metadata extraction", () => {
    ...
    it.each([true, false])(
      ...
      async (checkWorkingDirectoryStatus) => {
        ...
        expect(metadata.git.version).toBe(
          checkWorkingDirectoryStatus ? "0.0.2" : "0.0.1",
        );
      },
    );
  });
