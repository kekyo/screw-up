...
│       └── .gitignore             # Ignored `packageMetadata.ts` file
...
Setting `outputMetadataFile` to `true` will generate `packageMetadata.ts` and `packageMetadata.d.ts`. By default, `packageMetadata.ts` contains Git commit IDs, so it will be updated with each commit. Since this will cause Git to generate differences each time, we recommend excluding it with `.gitignore`.
