...
In addition to the TypeScript source file, a corresponding type definition file (.d.ts) is also generated for better TypeScript support.
By default, the extension of `outputMetadataFilePath` is changed to `.d.ts`.
You can customize the output file path by specifying `outputMetadataFileTypePath`:
...
Note: Since type definition files are generated early in the plugin lifecycle, before the actual metadata source code `packageMetadata.ts` is generated, there may be cases where it's unclear whether metadata items are actually output. For example, if a non-existent metadata item `foobar` is specified in `outputMetadataKeys`, a `declare` statement will be generated, but the corresponding `const foobar` variable will not be generated, resulting in a build error when referenced.
...
│       ├── packageMetadata.d.ts   # Auto-generated by `outputMetadataFile`
│       └── .gitignore             # Auto-generated to ignore `packageMetadata.ts`
...
Setting `outputMetadataFile` to `true` will generate `packageMetadata.ts` and `packageMetadata.d.ts`. By default, `packageMetadata.ts` contains Git commit IDs, so it is updated on every commit. screw-up now creates a `.gitignore` next to the metadata file (if missing) so the file stays out of version control, but keep the ignore entry if you customize the directory.
